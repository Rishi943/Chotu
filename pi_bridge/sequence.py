...
the motion lock) — no network gap between frames.

Ends on the LAST authored frame (WYSIWYG with the studio preview). It does NOT append a
library `stand` — that routine spreads then re-rises ("sit-and-stand"). If you want the
robot standing at the end, make the last frame a stand pose (every studio built-in does).
"""
import time
